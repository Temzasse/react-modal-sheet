...
function useModalEffect({
  ...
  const heightRef = React2.useRef(IS_SSR ? 0 : window.innerHeight);
  function setup() {
  ...
  React2.useEffect(() => {
    ...
    const removeChangeListener = y.on("change", (value) => {
      if (root) {
        ...
        const ty = react.transform(progress, [0, 1], [0, 24]);
        const s = react.transform(progress, [0, 1], [1, scale]);
        const borderRadius = react.transform(progress, [0, 1], [0, 10]);
  ...
  ({
    ...
    const onDragEnd = useEffectEvent((_, { velocity }) => {
      if (velocity.y > dragVelocityThreshold) {
        ...
        react.animate(y, snapTo, animationOptions);
